...
import { generateAccessToken, generateRefreshToken, setAccessToken, setRefreshToken } from '../../utils/Authentication/function/tokens'
...
const validUserAgent = (testAgent: Details | undefined, ownerAgents: Array<UserAgent>) => {
...
export const refreshToken = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const tokenSender = crypto.createHash('sha256').update(refresh_token).digest('hex')
    ...
    await RefreshToken.findByIdAndUpdate(tokenOwner._id, { $push: { history: tokenSender }, token: crypto.createHash('sha256').update(newRefreshToken).digest('hex') })
